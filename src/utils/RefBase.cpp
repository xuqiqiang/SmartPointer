...
public:
	...
	weakref_impl(RefBase* base) :
			mStrong(INITIAL_STRONG_VALUE), mWeak(0), mBase(base), mFlags(0), mStrongRefs(
					NULL), mWeakRefs(NULL), mTrackEnabled(
					!!DEBUG_REFS_ENABLED_BY_DEFAULT), mRetain(false) {
	}
	...
	void removeRef(ref_entry** refs, const void* id) {
		if (mTrackEnabled) {
			...
			ALOGE("RefBase: removing id %p on RefBase %p"
					"(weakref_type %p) that doesn't exist!", id, mBase, this);
...
void RefBase::weakref_type::decWeak(const void* id) {
	...
	if ((impl->mFlags & OBJECT_LIFETIME_WEAK) == OBJECT_LIFETIME_STRONG) {
		...
		if (impl->mStrong == INITIAL_STRONG_VALUE) {
			...
			ALOGD("Sheep::Freeing refs %p of old RefBase %p\n", this, impl->mBase);
			delete impl;
		}
	} else {
...
bool RefBase::weakref_type::attemptIncStrong(const void* id) {
	...
	while (curCount > 0 && curCount != INITIAL_STRONG_VALUE) {
		...
		if (android_atomic_cmpxchg(curCount, curCount + 1, &impl->mStrong)
				== 0) {
			break;
		}
